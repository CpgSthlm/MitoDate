...
def read_metadata(joint_tree_meta, fasta):
    """Extract taxon sets and root height lower bound from metadata."""
    df = pd.read_csv(joint_tree_meta, sep='\t')
    ...
    root_height_lower = pd.to_numeric(df['Age'], errors='coerce').max()
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s -f alignment.fasta -m metadata.tsv --subs_model HKY --root_mean 50000 --root_stdev 5000 --chain_length 10000000 --log_every 1000
  %(prog)s -f alignment.fasta -m metadata.tsv --subs_model "tRNA:HKY+G+I, rRNA:HKY+G+I, CDS:HKY+G+I" --split_partition --annotation genome.gff --root_mean 50000 --root_stdev 5000 --chain_length 10000000 --log_every 1000 -o output_dir/
        """
    )

    # Required arguments
    parser.add_argument('-f', '--fasta', required=True, help='Path to fasta alignment file')
    parser.add_argument('-m', '--joint_tree_meta', required=True, help='Path to joint tree metadata TSV file')
...
def main():
    ...
    fasta = args.fasta
    joint_tree_meta = args.joint_tree_meta
    gff = args.annotation
    ...
    required_files = [fasta, joint_tree_meta]
    ...
    print(f"Using joint tree metadata: {joint_tree_meta}")
    ...
    taxon_set, root_height_lower = read_metadata(joint_tree_meta, fasta)
    ...
    mTaxa.intro_taxa_joint(beast, fasta, joint_tree_meta)
    ...
    # Module 9 -- Priors
    mPriors.define_operators_joint(jointPrior, joint, MCMC, fasta, partition_file, split_partition,
